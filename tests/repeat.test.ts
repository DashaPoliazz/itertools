...
import repeat from "../lib/producers/repeat";

it("should produce the same item 'n' times", () => {
  const itemToRepeat = "a";
  const n = 3;
  const result = Array.from(repeat(itemToRepeat, n));
...
it("should work with different types of items", () => {
  ...
  const n1 = 2;
  const result1 = Array.from(repeat(numberToRepeat, n1));

  const objectToRepeat = { key: "value" };
  const n2 = 2;
  const result2 = Array.from(repeat(objectToRepeat, n2));
